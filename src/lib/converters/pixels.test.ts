...
describe("convertToPixels", () => {
  ...
  describe("edge cases", () => {
    it("should return Err for negative inputs", () => {
      ...
      assert(!result3.ok);
      assertEquals(result3.error.kind, AppErrorKind.NegativeInput);
    });
  ...
  describe("viewport-based units", () => {
    ...
    it("should convert Vmin to pixels", () => {
      ...
      assertEquals(
        result.value,
        Math.round(Math.min(VIEWPORT_WIDTH, VIEWPORT_HEIGHT) / 100),
      );
    });
    ...
    it("should convert Vmax to pixels", () => {
      ...
      assertEquals(
        result.value,
        Math.round(Math.max(VIEWPORT_WIDTH, VIEWPORT_HEIGHT) / 100),
      );
    });
